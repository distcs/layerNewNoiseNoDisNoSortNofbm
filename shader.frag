...
vec2 computeDisplacement(vec2 uv, float time) {
    ...
    float n = 1.0;//fbm(uv * noiseScale + time * noiseSpeed);
...
void main() {
    ...
    if(u_t < 0.5) {
        sortedUV.y = uv.y;//mix(displacedUV.y, sortValue, 0.04);
    } else {
        sortedUV.x = uv.x;//mix(displacedUV.x, sortValue, 0.04);
    }

    float grain = 1.0;//fbm(sortedUV * u_clear);
